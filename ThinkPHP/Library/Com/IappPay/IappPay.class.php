<?php
namespace Com\IappPay;
header("Content-Type: text/html; charset=utf-8");

class IappPay{

    const API_URI="http://ipay.iapppay.com:9999/payapi/order";
    ...
    public function __construct($appid = '', $appkey = '', $platpkey = ''){
        $this->appId    = $appid ?: C('IAPP_PAY_APPID');
        $this->appKey    = $appkey ?: C('IAPP_PAY_APPKEY');
        $this->platpkey = $platpkey ?: C('IAPP_PAY_PLATPKEY');
    }
     ...
     */
    public function parseResp($content,&$respJson) {
        ...
        if(array_key_exists("sign", $resp)) {
            //校验签名
            $pkey = $this->formatPubKey($this->platpkey);
     ...
     * return 返回报文
     */
    public function request_by_curl($postData, $userAgent) {
        $ch = curl_init();
     ...
     * return 返回组装后的报文
     */
    public function composeReq($reqJson) {
        //获取待签名字符串
        $content = json_encode($reqJson);
        //格式化key，建议将格式化后的key保存，直接调用
        $vkey = $this->formatPriKey($this->appKey);
        //生成签名
